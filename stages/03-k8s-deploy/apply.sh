...
VARS='$NAMESPACE $APP_NAME $FETCHER_APP_NAME $NEWS_APP_NAME $FETCHER_IMAGE $FETCHER_TAG $FETCHER_CONTAINER_PORT $FETCHER_REPLICAS $NEWS_IMAGE $NEWS_TAG $NEWS_CONTAINER_PORT $NEWS_REPLICAS $GRAFANA_IMAGE $GRAFANA_ADMIN_USER $GRAFANA_ADMIN_PASSWORD $GRAFANA_STORAGE_SIZE $INFLUXDB_IMAGE $INFLUXDB_BUCKET $INFLUXDB_ORG $INFLUXDB_ADMIN_TOKEN $INFLUXDB_URL $INFLUXDB_STORAGE_SIZE $INFLUXDB_ADMIN_USER $INFLUXDB_ADMIN_PASSWORD $INFLUXDB_RETENTION $IMAGE $TAG $CONTAINER_PORT $REPLICAS $EXTERNAL_NODE_IP $GRAFANA_NODEPORT $TICKERS'
